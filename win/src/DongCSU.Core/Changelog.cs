    ...
    [
        new ChangelogEntry
        {
            ...
            [
                ...
                "펫을 끌면 매달린 자세 추가 (끄는 방향과 빠르기에 따라 몸과 날개가 움직임)",
                "펫을 마구 흔들면 어지러워하는 자세 추가 (흔드는 도중에 바로)",
                "지치거나 탈진한 채로 걷도록 수정 (걸으면 말짱해지던 문제)",
                "조회가 끊긴 동안에는 펫이 돌아다니지 않도록 수정",
                "설정 창에서 조작부가 설명 글을 가리던 문제 수정",
                "주간 한도를 다 쓰면 마스코트·세션 링·세션 숫자를 회색으로 표시하도록 변경",
                "카운트다운·CPU 메모리 줄·버전 딱지에 설명 문구 추가",
                "펫 링과 버튼이 스르르 떠오르도록 변경",
                "펼침과 접힘 사이에서 창 크기가 부드럽게 옮겨가도록 변경 (펫은 곧바로)",
                "마우스를 움직이면 펫이 커서를 영영 피하지 않던 문제 수정",
                "펫이 커서를 한 번만 피하고 그 뒤로는 피하지 않던 문제 수정",
                "마스코트 아래 버튼 줄에서 링이 사라지고 커서도 피하지 않던 문제 수정",
                "배경 불투명도 기본값을 100%로 변경",
                "숨겨 두거나 화면이 잠긴 동안 마스코트 애니메이션을 멈추도록 개선",
                "걸을 때 눈을 너무 자주 깜빡이던 문제 수정",
                "마구 흔들어도 잘 어지러워하지 않던 문제 수정",
