...
public sealed class UsageMeter
{
    ...
    /// </param>
    public UsageMeter(MeterStore? store = null, TimeProvider? time = null, string? transcriptRoot = null)
    {
        this.store = store;
    ...
    {
        // **기록 폴더도 물리지 않는다.** 안 물리면 `root` 가 null 이라 사용자의 진짜
        // `~/.claude/projects` 를 보게 되는데, 측정 탭은 열릴 때 한 번 훑으므로
        // `--render-settings measure` 나 `--probe-layout` 이 남의 기록 200MB 를 훑고
        // 그 숫자가 고정값 화면에 섞여 든다. 있을 수 없는 자리를 줘서 아예 안 훑게 한다.
        var meter = new UsageMeter(store: null, time: time, transcriptRoot: NowherePath);
        meter.state = preview;
        return meter;
    }

    /// <summary>고정값 전용. 이 이름의 폴더가 있을 리 없어서 훑기가 늘 건너뛴다.</summary>
    private const string NowherePath = @"\\?\dong-csu-preview-no-transcripts";
    ...
    /// <summary>훑기 타이머를 켜 둘 때인지. 화면 쪽이 이걸 보고 <c>DispatcherTimer</c> 를 켰다 껐다 한다.</summary>
    public bool WantsScanning => State.IsCounting;

    /// <summary>토큰 세는 중. 파일을 훑는 동안 버튼이 두 번 눌리지 않게 하는 표시이기도 하다.</summary>
    ...
    /// 없어서, 표식 대조를 검사하려면 이 자리가 열려 있어야 한다.
    /// </summary>
    public Func<TokenScan, CancellationToken, Task<TokenScanResult>>? ScanRunner { get; set; }
    ...
    public void Start()
    {
        ...
        var offsets = ClaudeCodeUsage.EndOffsets(root);
    ...
    public void Record(UsageSnapshot snapshot)
    {
        ...
        lock (gate)
        {
            ...
            foreach (var limit in LimitsOf(snapshot))
            {
                ...
                {
                    next.Tracks[limit.Id] = new LimitTrack
                    {
                        Title = limit.Title,
                ...
                if (needsRebaseline)
                {
                    ...
                    // (`next` 의 트랙은 이미 복사본이라 그 자리에서 고쳐도 밖으로 나간
                    // 상태나 목록에 남긴 기록이 함께 바뀌지 않는다.)
                    track.LastPercent = limit.Percent;
                    track.LastResetsAt = limit.ResetsAt;
                }
                else
                {
                    next.Tracks[limit.Id] = Advance(track, limit);
                }
            }
    ...
    public async Task ScanTokensAsync(CancellationToken cancellationToken = default)
    {
        ...
        lock (gate)
        {
            ...
            isScanning = true;
            // TokenScan 이 생성자에서 사전을 복사한다 — 락 안에서 만들어야 그 복사가
            // 배경 훑기와 겹치지 않는다.
    ...
    ///
    /// **인자를 고치지 않고 새 상태를 돌려준다** — 진단 통로(<c>--probe-meter scan</c>)가
    /// 이걸 그대로 써서 확인 통로와 실제 동작이 갈라질 수 없다.
    ///
    ...
    {
        var next = state.Copy();
        next.Offsets = ClaudeCodeUsage.WithPathComparer(result.Offsets);
        next.SeenIds = new HashSet<string>(result.SeenIds, StringComparer.Ordinal);
    ...
    /// <summary>기록 폴더가 있는지. 없으면 아예 안 훑는다(WSL 만 쓰는 사람이 여기 걸린다).</summary>
    private bool RecordsAvailable
        => root is null ? ClaudeCodeUsage.IsAvailable : Directory.Exists(root);
