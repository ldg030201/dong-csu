...
| 트레이 아이콘 · 버전 딱지 | 평소 색 | **보라** · `2.5.0 test` |
| 마스코트 | 평소 색 | **평소 색** (2.5.0 부터 — 아래를 봐라) |

**마스코트는 판에 상관없이 같은 색이다.** 한동안 테스트판에서 보라로 물들였는데,
그러면 캐릭터가 캐릭터로 안 보인다 — 새로 그린 그림을 확인하려고 띄운 테스트판에서
정작 그 색을 못 본다. 두 판을 가르는 일은 트레이 아이콘과 버전 딱지가 한다.
맥 2.5.2 가 같은 이유로 뺐고 거기서도 메뉴바 아이콘만 남겼다.
...
| 설정 창 | 여섯 탭, 다크·라이트, 최대화까지 확인 (지금은 일곱이다) |
...
| 지친 채로 걷기 | 세션 75% 넘은 상태에서 걸어도 눈꺼풀이 그대로다 |
| 보기 전환 | 펼침↔접힘은 7~8칸에 걸쳐 미끄러지고, 펫은 중간 크기 없이 곧바로 바뀐다 |
| 설정 창 배치 | 여섯 탭 전부에서 조작부가 설명 글을 안 가린다 (지금은 일곱이다) |
...
DongCSU.exe --probe-login    # 재로그인을 누르면 무엇이 뜨는지 (후보를 전부 찍는다)
DongCSU.exe --probe-layout   # 탭마다 세로로 몇 pt 인지, 가로로 잘리는 곳은 없는지
DongCSU.exe --probe-hud      # 카드가 모든 조합(보기 3 × 모델별 × CPU × 배율 4)에서 안 넘치는지
DongCSU.exe --probe-perch    # 창에 붙는 규칙 (selftest) · 지금 떠 있는 창 (windows)
DongCSU.exe --probe-pet      # 다른 화면으로 넘어가는 계산
DongCSU.exe --probe-mascot   # 시트가 어떻게 구워져 있는지 재서 칸 표와 대조 (캐릭터 전부)
...
**문서 그림도 여기서 뽑는다**(`docs/images/`). 테스트 바이너리로 뽑아도 마스코트는
**정식판과 같은 색**이다 — 2.5.0 부터 판에 따라 색을 돌리지 않는다. 버전 딱지의 `test`
만 남으므로, 그것까지 없애고 싶으면 정식판으로 뽑아라.
...
끌어다 놓으면 다른 앱 창 테두리에 앉거나 매달린다(맥 2.5.0~2.5.1을 옮긴 것).
**경계가 갈려 있다.**

| | 어디 |
| --- | --- |
| 어디에 붙을지 고르기 · 가림 판정 | `Core/Pet/PerchFinder` — 가짜 창 목록으로 테스트된다 |
| 자리 계산(원점 · 잠기는 깊이) | `Core/Pet/PerchLayout` — 잉크 상자를 **숫자로 받는다** |
| 붙어 있는 상태 | `Core/Pet/PetMotion` 의 `Perched` |
| 창 조사 · DIP 환산 | `App/Hud/WindowSurvey` (`user32`·`dwmapi`) |
| 잉크 재기 · 막대 창 · 층 | `App/Hud/HudView`·`PerchHint`·`HudWindow` |

**`GetWindowRect` 를 쓰지 마라.** 윈도우 10부터 DWM 이 창 바깥에 보이지 않는 잡는
테두리를 두는데(좌·우·아래 7~8px, 위는 없다) 그것까지 센다 — 그대로 붙이면 옆과
아래에서만 펫이 떠 있고 위에서만 맞는다. `DWMWA_EXTENDED_FRAME_BOUNDS` 를 쓴다.

**맥과 세로가 뒤집혀 있다.** 맥은 y 가 위로 크다. 네 변의 부호를 옮겨 적을 때 제일
틀리기 쉬운 자리라 `PerchLayoutTests` 가 변마다 하나씩 굳혀 놨다.

안 붙는다는 말을 들으면 **기록부터 본다** — 놓을 때마다 왜 안 붙었는지 창·변마다 한 줄씩
남는다(`%APPDATA%\DongCSU\log.txt`). 지금 떠 있는 창은 `--probe-perch windows`,
규칙 자체는 `--probe-perch selftest` 다.
...
```bash
DongCSU.exe --probe-mascot            # 시트로 도는 캐릭터 전부
DongCSU.exe --probe-mascot raccoon    # 하나만
```

시트를 실제로 재서 칸 표와 맞대 본다. **CI 가 이걸 부른다**(`build-win.yml`) —
옮겨 적은 것이 어긋나면 여기서 걸린다.

**인자를 안 주면 전부 돈다.** 하나만 재면 새 캐릭터가 조용히 빠진다 — 칸 표는
캐릭터마다 다시 쓰지 않고 하나를 나눠 쓰므로, 라쿤 시트가 부엉이와 다르게 구워져
있으면 라쿤에서만 어긋난다.

**`beta` 캐릭터는 어긋나도 실패로 안 친다.** 이 검사가 잡으려는 것은 칸 표를 잘못
옮겨 적은 것인데, 실제로 걸리는 것에는 그리는 쪽이 몇 px 어긋나게 그린 것도 섞인다.
다듬는 중이라고 못 박아 둔 캐릭터에서는 뒤쪽이 훨씬 잦고, 그것 때문에 CI 가 빨개지면
부엉이의 진짜 표 오류까지 같이 묻힌다. 줄은 그대로 찍히므로 눈으로는 보인다.
