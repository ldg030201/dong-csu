...
입력 피하기는 **권한이 있고 없고에 따라 하는 일이 달라진다.** 없다고 죽지는 않지만,
없으면 눈에 띄게 무디다.

| | 아는 것 | 하는 일 |
| --- | --- | --- |
| **정밀** (손쉬운 사용 권한) | 캐럿의 화면 좌표 | 글자가 닿을 참에 오른쪽·아래로 |
| **거친** (권한 없음) | 앞 창의 크기와 자리 | 쓰기 시작할 때 그 창 위에 있으면 창 밖으로 |

거친 쪽은 캐럿이 아직 멀리 있어도 미리 비킨다. 창 밖으로 한 번 나가면 더 밀려날 데가
없어서 타이핑이 이어져도 다시 움직이지 않고, 창이 화면을 다 덮었으면(전체화면) 갈 데가
없어서 그냥 있는다. 권한이 있으면 **정밀한 쪽이 늘 이긴다** — 캐럿이 창 저쪽 끝에
있는데 창 밖으로 튀어나가면 왜 움직였는지 알 수 없다.

**그래서 권한을 적극적으로 묻는다.** 조용히 거친 모드로 돌면 사용자는 정확도가
낮아진 게 아니라 **고장난 줄 안다.** 펫 모드에서 이 기능이 실제로 돌기 시작할 때 묻는다.
꺼 두면 묻지 않는다.

다만 **창을 띄우는 건 살면서 딱 한 번뿐이다.** 손쉬운 사용 권한은 **코드 서명에 걸려
있어서**, 앱을 업데이트하면 바이너리가 달라지고 허용해 둔 게 풀린 채로 뜬다. 그때마다
창을 띄우면 허락을 받아낼 때까지 조르는 앱이 된다. 물어봤다는 사실을 저장해 두고,
풀린 건 **메뉴바 메뉴와 설정 창에 조용히 남겨** 다시 허용할지는 사용자가 정하게 한다.

> 개발 중에는 이게 더 자주 일어난다. 테스트판은 ad-hoc 서명이라 **소스를 고쳐 다시
> 빌드할 때마다 서명 해시가 달라진다.** 허용해 둔 게 그때마다 풀리므로,
> `dong-mcu --probe-accessibility`로 지금 붙어 있는지 확인하고 다시 허용한다.
