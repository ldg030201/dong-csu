...
캐럿이 어디 있는지는 **손쉬운 사용 권한 말고는 알 방법이 없다.** 권한 없이 앞 창의
크기·자리만 보고 짐작하는 방식도 만들어 봤지만 걷어냈다 — 캐럿이 창 저쪽 끝에 있는데
창 밖으로 튀어나가니, 비켜준 게 아니라 **제멋대로 도는 것**으로 보였다.
어설프게 도는 것보다 안 도는 게 낫다.

그래서 권한이 없으면 이 기능은 **아무것도 하지 않고**, 그 사실을 화면에 남긴다.

**권한은 한 번만 묻는다.** 거절한 사람에게 계속 창을 띄우면 허락을 받아낼 때까지 조르는
앱이 된다. 딱 하나 예외가 있는데, **잘 쓰던 권한이 저절로 풀린 경우**다 — 권한은 코드
서명에 걸려 있어서 앱을 업데이트하면 풀린다. 그건 거절과 달리 사용자가 한 일이 아니라
한 번 더 묻는다(`hadAccessibility`).
