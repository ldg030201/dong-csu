...
**읽기는 안 묻는데 쓰기는 묻는다.** 읽기는 Apple이 서명한 `/usr/bin/security` 를 거쳐서
신원이 고정되지만, 쓰기는 우리 바이너리가 직접 한다. 그리고 이 앱은 **ad-hoc 서명**이라
신원이 없어서 macOS가 바이너리 해시로 알아본다 — **다시 빌드할 때마다 해시가 바뀌므로
버전을 올릴 때마다 한 번씩 다시 묻는다.** 개발 중에는 빌드할 때마다 뜬다.

`security` 로 쓰면 그 문제가 없지만 **쓸 수 없다.** 값을 인자로 넘기면 프로세스 목록에
토큰이 드러나고, 표준 입력으로 먹이면 **128바이트에서 잘린다**(항목이 3KB 넘는다).
실제로 재 보고 버린 길이다.

그래서 **거절당하면 이레 동안 다시 묻지 않는다**(`KeychainWriteBack`). 이게 없으면
갱신할 때마다, 즉 여덟 시간에 한 번씩 같은 창이 뜬다. 그동안 우리는 우리 사본으로
계속 돌고, Claude Code 쪽 토큰만 낡은 채로 남는다 — 그게 거절의 뜻이다.
