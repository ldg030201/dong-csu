...
붙인다(`1.0.0.1`). 첫 번째 자리는 크게 뜯어고쳤을 때만 올리고, 두 번째 자리는 10을
넘어도 그대로 올린다(`1.9.0` 다음은 `1.10.0`). 내보내기 전에
[`Changelog.swift`](Sources/DongMCU/Changelog.swift)의 맨 위 항목에 버전과 날짜를
채운다. 설정 창의 변경 내역 탭이 그걸 그대로 보여준다.
